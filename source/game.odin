...
game_init :: proc() {
  ...
  // cube
  g.pipelines[.Cube] = sg.make_pipeline(default_pip_desc)
  build_shape(
    .Cube,
    sshape.Box{width = 3, height = 3, depth = 3, tiles = 1, random_colors = true},
  )
...
game_frame :: proc() {
  ...
  // cube
  sg.apply_pipeline(DEBUG_LINES ? g.pipelines[.Primitive] : g.pipelines[.Cube])
