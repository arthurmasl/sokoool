...
game_init :: proc() {
  ...
  // buf = sshape.build_plane(buf, {width = 1.0, depth = 5.0, tiles = 0, random_colors = false})
...
game_frame :: proc() {
  ...
  model :=
    linalg.matrix4_translate_f32({linalg.cos(time) * 2, 0, linalg.sin(time)} * 2) *
    linalg.matrix4_translate_f32({0, 0.5, 0}) *
    linalg.matrix4_rotate_f32(linalg.RAD_PER_DEG * 90 * time, {0, 1, 0})
